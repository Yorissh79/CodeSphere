...
export const getAllQuizzes = async (_req: Request, res: Response) => {
    try {
        const quizzes = await quizModel.find().select("title tags timeLimit");
...
export const getQuizWithQuestions = async (req: Request, res: Response):Promise<any> => {
    try {
        const quiz = await quizModel.findById(req.params.id).populate("questionIds");
