...
import {useCheckGoogleAuthQuery} from "../../services/authCheck.ts"; // Assuming you'll create this new RTK Query hook

const GoogleAuthHeader: React.FC = () => {
    ...
    // This new query will check Google authentication status and user role
    const {
        data: authData,
        isError: authError,
    ...
    useEffect(() => {
        ...
        if (authError) {
            // If authentication fails or no Google auth data, redirect to login
            setRender(false);
        ...
        if (authData?.user) {
            // If authenticated, set render to true and navigate based on role
            setRender(true);
            if (authData.user.role === "student") {
                navigate("/google/student");
            } else if (authData.user.role === "teacher") {
                navigate("/google/teacher");
            } else {
                // Handle other roles or default to a generic dashboard
                navigate("/google/dashboard");
            }
        } else {
            // No user data, so not authenticated
            setRender(false);
            navigate("/registration/login");
        }
    }, [authData, authError, authLoading, navigate]);
    ...
    const fullName = `${authData?.user?.name || "User"} ${authData?.user?.surname || ""}`;
    ...
    const navigationItems = [
        // These can be adjusted based on what Google authenticated users should see
        {path: "/google/misses", label: "Misses", icon: Clock},
        {path: "/google/code-editor", label: "Code Editor", icon: Code},
        {path: "/google/quiz", label: "Quiz", icon: FileText},
        {path: "/google/task", label: "Task", icon: GraduationCap},
    ];

    const mobileNavigationItems = [
        {
            path: authData?.user?.role === "student" ? "/google/student" : "/google/teacher",
            label: "Dashboard",
            icon: Home
        },
        {path: "/google/profile", label: "Profile", icon: User},
        ...navigationItems,
    ];
            ...
            {render ? (
                ...
                >
                    <div
                        ...
                        >
                            <Link
                                to={authData?.user?.role === "student" ? "/google/student" : "/google/teacher"}
                            ...
                            >
                                ...
                                <div className="flex flex-col justify-center">
                                    ...
                                    <span className="text-xs lg:text-sm text-slate-500 dark:text-slate-400 font-medium">
                                        {authData?.user?.role === "student" ? "Student Portal" : "Teacher Portal"}
                                    </span>
                                </div>
                            </Link>
                        </motion.div>
